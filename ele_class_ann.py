...
TRAIN_DATASET_PATH = '/home/dell/dataset/train_ele' # the dataset file or root folder path.
TEST_DATASET_PATH = '/home/dell/dataset/test_ele'
...
file_to_write= open("/home/dell/tensorflowCode/roc.txt","w")
# Reading the dataset
...
loss_op = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
    logits=logits_train, labels=Y_train),name="loss")
optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate,name="optimizer")
train_op = optimizer.minimize(loss_op,name="train_op")

# Evaluate model (with test logits, for dropout to be disabled)
correct_pred = tf.equal(tf.argmax(logits_test, 1), tf.cast(Y_train, tf.int64),name="correct_pred")
...
with tf.Session() as sess:
    ...
    file_to_write.write("FPR= ["+",".join(str(x) for x in FPR)+"]"+"\n")
    file_to_write.write("TPR= ["+",".join(str(x) for x in TPR)+"]"+"\n")
    TNR = 1-FPR
    FNR = 1-TPR
